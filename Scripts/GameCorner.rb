...
def gameCornerShop(index)
  ...
  check_prizes = [:GOLDENAXE,:GOLDENHAMMER,:GOLDENSURFBOARD,:GOLDENGAUNTLET,:GOLDENSCUBAGEAR,:GOLDENWINGS,:GOLDENJETPACK,:GOLDENDRIFTBOARD,:GOLDENCLAWS,:EXPSHARE]
  ...
  if badges>=4
    for i in 0...post5_prizes.length
      if !$PokemonBag.pbHasItem?(post5_prizes[i])
        prizes[2].insert(-1,post5_prizes[i])
        prices[2].insert(-1,post5_prices[i])
      end
    end
  end
  if badges>=5 && !$PokemonBag.pbHasItem?(:EXPALL) && !$PokemonBag.pbHasItem?(:EXPALLOFF)
