...
#### SARDINES - v17 - END
  ...
  begin
    pbBGMPlay(bgm) if bgm.name.chop.chop.chop.chop != playingBGM.name
  rescue
    pbBGMPlay(bgm)
  end
...
################################################################################
  if trainerid>=0
    opposite= 99
    if $game_variables[646]==65 
      tbargraphic = sprintf("Graphics/Transitions/vsBar%s_%d",getConstantName(PBTrainers,trainerid),opposite) rescue nil
      tbargraphic = sprintf("Graphics/Transitions/vsBar%d_%d",trainerid,opposite) if !pbResolveBitmap(tbargraphic)
      tgraphic    = sprintf("Graphics/Transitions/vsTrainer%s_%d",getConstantName(PBTrainers,trainerid),opposite) rescue nil
      tgraphic    = sprintf("Graphics/Transitions/vsTrainer%d_%d",trainerid,opposite) if !pbResolveBitmap(tgraphic)
    else
    ...
    begin
    if pbResolveBitmap(tbargraphic) && pbResolveBitmap(tgraphic) && !$game_switches[145]
      ...
      player.mirror = true if $game_switches[1235]
      player.x      = -xoffset
...
def pbWildBattleBoss(species,level,variable=nil,canescape=true,canlose=false)
  if (Input.press?(Input::CTRL) && $DEBUG) || $Trainer.pokemonCount==0
    if $Trainer.pokemonCount>0
      Kernel.pbMessage(_INTL("SKIPPING BATTLE..."))
    end
  ...
  battle.internalbattle=true
  if $PokemonGlobal.partner
    battle.items=$PokemonGlobal.partner[4]
  end
...
def pbDoubleWildBattle(species1,level1,species2,level2,variable=nil,canescape=true,canlose=false)
  ...
  shinyMon = genwildpoke.isShiny?
  shinyMon2 = genwildpoke2.isShiny?
  Events.onStartBattle.trigger(nil,genwildpoke)
  Events.onStartBattle.trigger(nil,genwildpoke2)
  scene=pbNewBattleScene
  if $PokemonGlobal.partner
...
def pbEvolutionCheck(currentlevels)
  ...
  for pokemon in $Trainer.party
    currentlevel = currentlevels.assoc(pokemon.personalID) #modified
...
def Kernel.pbPickup(pokemon)
  ...
  Achievements.incrementProgress("PICKUP",1)
end
...
def Kernel.pbConveyorMove(event=nil)
  event=$game_player if !event
  return if !event
  return if (pbGetTerrainTag(event)!=PBTerrain::DownConveyor &&  
             pbGetTerrainTag(event)!=PBTerrain::LeftConveyor && pbGetTerrainTag(event)!=PBTerrain::RightConveyor && 
             pbGetTerrainTag(event)!=PBTerrain::UpConveyor)
  $PokemonGlobal.sliding=true
  oldwalkanime=event.walk_anime
  event.straighten
  event.pattern=0
  ...
  loop do
    ...
    when PBTerrain::DownConveyor
      direction=2
    when PBTerrain::LeftConveyor
      direction=4
    when PBTerrain::RightConveyor
      direction=6
    when PBTerrain::UpConveyor
      direction=8
    end
    break if (pbGetTerrainTag(event)!=PBTerrain::DownConveyor && pbGetTerrainTag(event)!=PBTerrain::LeftConveyor && 
              pbGetTerrainTag(event)!=PBTerrain::RightConveyor && pbGetTerrainTag(event)!=PBTerrain::UpConveyor)
    break if !event.passable?(event.x,event.y,direction)
    case pbGetTerrainTag(event)
    when PBTerrain::DownConveyor
...
Events.onStepTakenFieldMovement+=proc{|sender,e|
  ...
  if pbGetTerrainTag(event,true)==PBTerrain::Grass  # Won't show if under bridge
    $scene.spriteset.addUserAnimation(GRASS_ANIMATION_ID,event.x,event.y,true) rescue nil
  elsif event==$game_player && currentTag==PBTerrain::WaterfallCrest
    # Descend waterfall, but only if this event is the player
    Kernel.pbDescendWaterfall(event)
  elsif (currentTag==PBTerrain::DownConveyor || currentTag==PBTerrain::LeftConveyor ||
         currentTag==PBTerrain::RightConveyor || currentTag==PBTerrain::UpConveyor) && !$PokemonGlobal.sliding
...
def Kernel.pbReceiveItem(item,quantity=1,plural=nil,silent=false)
  if item.is_a?(String) || item.is_a?(Symbol)
    item=getID(PBItems,item)
  end
  return false if !item || item<=0 || quantity<1
