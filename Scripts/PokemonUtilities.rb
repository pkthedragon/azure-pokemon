...
def pbItemIconFile(item)
  ...
  if item==0
    ...
    moveid = $ItemData[item][ITEMMACHINE]
...
def pbGenerateEgg(pokemon,text="")
  ...
  pokemon.eggsteps=$pkmn_dex[pokemon.species][10]
...
def pbSize(pokemon)
  baseheight=$pkmn_dex[pokemon.species][14]
...
def pbHasEgg?(species)
  ...
  compat1=$pkmn_dex[species][13]   # Get egg group of this species
...
for i in 0...6
  ...
  if totalev+252>510
      ...
      Kernel.pbMessage(_INTL("Hm, it seems that your {1} already has two maximized stats. Which one would you like to replace?",PBSpecies.getName(pkmn.species)))
      cmd=0
