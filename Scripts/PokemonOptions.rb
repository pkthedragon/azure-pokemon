...
class PokemonSystem
  ...
  attr_accessor :photosensitivity
  ...
  def difficulty
    v = @difficulty
    if v.nil? && defined?($game_variables) && $game_variables
      v = $game_variables[200]
    end
    v = 1 if v.nil?
    v = 0 if v < 0
    v = 2 if v > 2
  ...
  def difficulty=(value)
    ...
    value = 2 if value > 2
    @difficulty = value
    $game_variables[200] = value if defined?($game_variables) && $game_variables
  end

  def expall
    v = @expall
    if v.nil? && defined?($game_switches) && $game_switches
      v = $game_switches[1501]
    end
  ...
  def expall=(value)
    v = (value ? true : false)
    @expall = v
    $game_switches[1501] = v if defined?($game_switches) && $game_switches
  end

  def photosensitivity
    v = @photosensitivity
    if v.nil? && defined?($game_switches) && $game_switches
      v = $game_switches[1425]
    end
    # Default OFF if unset (intro prompt may set it)
    return false if v.nil?
    return v ? true : false
  end

  def photosensitivity=(value)
    v = (value ? true : false)
    @photosensitivity = v
    $game_switches[1425] = v if defined?($game_switches) && $game_switches
  end
...
class PokemonOptionScene
 ...
 # or delete it.  The game's options may be placed in any order.
    @PokemonOptions=[
       ...
       #---------------------------------------------------------------------------
       EnumOption.new(_INTL("Difficulty"),[_INTL("Easy"),_INTL("Normal"),_INTL("Intense")],
          proc { $PokemonSystem.difficulty },
          proc {|value| $PokemonSystem.difficulty = value }
       ),
       EnumOption.new(_INTL("EXP All"),[_INTL("On"),_INTL("Off")],
          proc { $PokemonSystem.expall ? 0 : 1 },
          proc {|value| $PokemonSystem.expall = (value == 0) }
       ),
       EnumOption.new(_INTL("Photosensitivity"),[_INTL("Off"),_INTL("On")],
          proc { $PokemonSystem.photosensitivity ? 1 : 0 },
          proc {|value| $PokemonSystem.photosensitivity = (value == 1) }
       ),
