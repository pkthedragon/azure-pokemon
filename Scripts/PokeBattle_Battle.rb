...
module PokeBattle_BattleCommon
  ...
  def pbThrowPokeBall(idxPokemon,ball,rareness=nil,showplayer=false)
    ...
    if @opponent && ((!pbIsSnagBall?(ball) || !battler.isShadow?) || ((pbIsSnagBall?(ball) || battler.isShadow?) && $game_switches[1235]==true))
      ...
      if no_catch || battler.issossmon || (battler.isbossmon && (!battler.capturable || battler.shieldCount > 0))
        pbDisplay(_INTL("The Pokémon knocked the ball away!"))
        pbBallFetch(ball)
        return
      end
...
################################################################################
  ...
  def pbOnActiveOne(pkmn,onlyabilities=false)
    ...
    if !onlyabilities
      ...
      # Presents
      if side.effects[PBEffects::Presents] > 0 && !battler.isFainted? &&
    ...
    if $fefieldeffect == 16
      if @eruption
        ...
        for i in priority
          ...
          if (i.pbOwnSide.effects[PBEffects::ToxicSpikes] || 0) > 0
...
##### KUROTSUNE - 009 - END
    for i in priority
      ...
      if @field.effects[PBEffects::SosBuffer] == 0 && (@doublebattle && i.isbossmon && i.pbPartner.issossmon)
        @field.effects[PBEffects::SosBuffer] = i.sosDetails && i.sosDetails[:sosBuffer] ? i.sosDetails[:sosBuffer] : 3
      end
    end
    # sosBuffer
