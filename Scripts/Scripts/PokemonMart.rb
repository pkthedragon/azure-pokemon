...
class PokemonMartAdapter
  ...
  def canSell?(item)
    return getPrice(item,true)>0 && !pbIsImportantItem?(item) || !pbIsCrest?(item)
  end

  def showQuantity?(item)
    return !pbIsImportantItem?(item) || !pbIsCrest?(item)
  end
    ...
    loop do
      ...
      if added!=quantity
        ...
        for i in 0...@stock.length
          if (pbIsImportantItem?(@stock[i]) || pbIsCrest?(@stock[i])) && $PokemonBag.pbQuantity(@stock[i])>0
            @stock[i]=nil
          end
        end
        @stock.compact!
        Achievements.incrementProgress("ITEMS_BOUGHT",quantity)
        ...
        if $PokemonBag
          if quantity>=10 && isConst?(item,PBItems,:POKEBALL) && 
            ...
            if @adapter.addItem(getConst(PBItems,:PREMIERBALL))
              Achievements.incrementProgress("ITEMS_BOUGHT",1)
...
def pbPokemonMart(stock,speech=nil,cantsell=false)
  for i in 0...stock.length
    ...
    if !stock[i] || stock[i]==0 ||
       ((pbIsImportantItem?(stock[i]) || pbIsCrest?(stock[i])) && $PokemonBag.pbQuantity(stock[i])>0)
      stock[i]=nil
    end
  end
  stock.compact!
